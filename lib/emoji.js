...
;(function() {
	 ...
	 * (ie. `:)`)
	 * 
	 * @param {function} fn a function that is evaluated on each replacement
	 *
	 * @returns {string} A new string with all emoticons in `str`
	 * replaced by their colon string representations (ie. `:smile:`)
	 */
	emoji.prototype.replace_emoticons_with_colons = function(str, fn){
		var self = this;
		self.init_emoticons();
		var _prev_offset = 0;
	 ...
	 */
	emoji.prototype.replace_unified_with_colons = function(str, fn){
		var self = this;
		self.init_unified();
		...
		var fnGetAndReturn = (v, m) =>
		{
			var v = self.data[v][3][0];
			return v ? fn(':'+v+':') : m;
		}
	 ...
	 * replaced by their colon string representations (ie. `:smile:`)
	 */
	emoji.prototype.toColons = function(str, fn){
		var self = this;

		str = self.replace_emoticons_with_colons(str, fn);
		str = self.replace_unified_with_colons(str, fn);
	 ...
	 */
	emoji.prototype.toUnicode = function(str, fn){
		var self = this;
		self.init_colons();
		...
		return str.replace(self.rx_colons, function(m){
			...
			if (idx.indexOf('::skin-tone-') > -1){
				...
				if (val)
					...
					return fnGetAndReturn(skin_val, m);
			}
